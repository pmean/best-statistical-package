...
SAS has a long histroy of supporting its users through conferences. The first SAS conference was held in 1976 in Kissimmee, Florida. Every conference has published proceedings available for free on the internet and these papers are a great resource both for beginners and advanced students.
...
One of the great appeals of Stata was its interest in publishing cutting edge research. The Stata Technical Bulletin, started in 1991 and replaced by the Stat Journal ten years later, offered a chance for academically oriented researchers to share advanced statistical methodology combined with Stata macros.

Stata has nurtured a large user group community and has held meetings 
...
I love R because the code that you learn to manipulate data is the same code that you use to manipulate output and the same code that you use to write macros. This is unique among all the packages that we talk about today (with the possible exception of Python, see below).
...
I listed the fact that R is free among the disadvantages. It is actually the first thing that other people cite about R, but free software is not free. You pay more for free software in labor, typically. The help facilities in 
R are good, but they can't compare to the professionally produced documentation of the other three packages. R does testing for the base package, and people who write user contributed packages usually include reasonable tests. But you still find yourself worrying about QC. Some packages get orphaned over time.
...
Ther are lots of books written about R. I don't have a favorite, but everyone that I've ever read has been quite good.
...
Sa provides a broad and comprehensive coverage of statistical methodologies. They are not always first, but they are early, and their work is well vetted before it is released.
...
SPSS does a great job of making the same features available in programming statements (SPSS syntax) and through a menu system. I have succeffuly trained many people to use SPSS who would never be capable of using a programming language and after just a little bit of work, they can do a fair amount of work independently.

For two topics that are especially difficult in any statistical packages, date/time manipulations and restructuring longitudinal data, SPSS offers wizrds to guide you through the various options.
...
There are lots of examples like this error, and you should visit the EUSpRIG (European Spreadsheet Risk Interest Group) website if you don't believe me.
...
Graphs in Excel almost never look nice on the first run through, and customizing them is difficult. It is easy to select the wrong range and get an incorrect graph. The graphic types in Excel are limited and include a lot of puffery like 3D bar charts and exploding pie wedges.
...
Git integrates easily with R, works okay with others.

Jupyter notebooks offer a literate programming interface. Good for reproducible research.

SQL is a very easy language to learn, but also very powerful. Ideal for large and complex data structures.
